locals {
  bastion_address_space = cidrsubnet(var.base_address_space, 4, 0) # the last part is the index and this is gotten by doing 2^(borrowed bits) and then calculating the index
